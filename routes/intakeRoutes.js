...
router.post('/receive-data', async (req, res) => {
  ...
  try {
    ...
    const extractedData = {
      caller: (caller_id && caller_id !== "unknown") ? caller_id : null,
    ...
    // 1. Try to find by call_sid if available - this is the most reliable method
    if (extractedData.callSid) {
      try {
        ...
        if (callResult.rows.length > 0) {
          ...
          // Always update extractedData caller with the found phone number
    ...
    // 2. If we couldn't find by call_sid and we have a valid caller_id, try by caller_id
    ...
    if (extractedData.callSid) {
      try {
        ...
        await client.query(`
          ...
          VALUES ($1, $2, $3, NOW())
          ON CONFLICT (call_sid) DO UPDATE SET status = $3
    ...
    const insertResult = await client.query(`
      ...
      (phone_number, contact_id, user_id, communication_style, values, professional_goals, 
       partnership_expectations, raw_transcript, created_at)
      VALUES 
      ($1, $2, $3, $4, $5, $6, $7, $8, NOW())
      RETURNING id
    `, [
